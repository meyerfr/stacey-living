...
class Api::V2::WebhooksController < ActionController::Base
    ...
    begin
      ...
      if event['current']['status'] == 'lost'
        ...
        if event['meta']['object'] == 'deal'
          ...
          when 'invite send'
            # create Booking and Send invite
            RestClient.post(
              stacey_api_url("users/#{deal.user.id}/bookings"),
          ...
          when 'deposit received'
            BookingMailer.deposit_received(deal)
  ...
  def stacey_api_url(action)
    "https://join.stacey-living.de/v2/#{action}"
  end
