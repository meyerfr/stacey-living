class Project::StepsController < ApplicationController
  ...
  def project_params(step)
    permitted_attributes = case step
                           when "project_info"
                             [:name, {photos: []}, descriptions_attributes: [:id, :field, :content], join_amenities_attributes: [:id, :amenity_id, :name, :_destroy]]
                           when "address"
                           ...
                           when "rooms"
                             [roomtypes_attributes: [:_destroy, :id, :name, :size, {photos: []}, rooms_attributes: [:id, :_destroy, :intern_number, :house_number], descriptions_attributes: [:id, :field, :content], prices_attributes: [:id, :duration, :amount], join_amenities_attributes: [:id, :amenity_id, :name, :_destroy]]]
                           end
