...
task :send_mails => :environment do
  ...
  to_be_reminded_bookings = Booking.where(booking_process_invite_send: [Date.today - 5.days, Date.today - 10.days], state: ['deposit outstanding', 'bookingFee payment failed'])
  ...
  # not_received_deposit_bookings = Booking.where("booking_process_completed_on =  AND state = ?", Date.today - 2.days, 'deposit outstanding')
